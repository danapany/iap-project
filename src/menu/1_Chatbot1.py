...
search_index = os.getenv("INDEX_REBUILD_NAME")
...
장애현상은 현상(symptom) 을 참고하고 없으면 영향도(effect)를 참고해서주세요
...
# LLM 기반 쿼리 타입 분류 함수
def classify_query_type_with_llm(azure_openai_client, query, model_name):
...
def process_query_with_enhanced_filtering(query, query_type=None):
    """서비스명 포함 검색을 지원하는 개선된 쿼리 처리"""
    with st.chat_message("assistant"):
        # LLM 기반 쿼리 타입 자동 분류
        if query_type is None:
            with st.spinner("🤖 질문 유형 분석 중..."):
                query_type = classify_query_type_with_llm(azure_openai_client, query, azure_openai_model)
...
if all([azure_openai_endpoint, azure_openai_key, search_endpoint, search_key, search_index]):
    ...
    if init_success:
        ...
        if user_query:
            ...
            process_query_with_enhanced_filtering(user_query)
        ...
        **필요한 환경변수:**
        ...
        env_status = {
            ...
            "INDEX_REBUILD_NAME": "✅" if search_index else "❌"
        }
