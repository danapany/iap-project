...
if all([azure_openai_endpoint, azure_openai_key, search_endpoint, search_key, search_index]):
    ...
    if init_success:
        ...
        with st.container():
            ...
            with input_col1:
                service_name = st.text_input("서비스명을 입력하세요", placeholder="예: ERP, 마이페이지, 패밀리박스")
            ...
            with col1:
                if st.button("🔧 서비스와 현상에 대해 복구 방법 안내", key="repair_btn"):
                    if validate_inputs(service_name, incident_symptom):
                        ...
                        st.session_state.sample_query = f"{search_query}에 대한 장애를 해소하기 위한 근본적인 복구방법 중에 가장 효과적인 방법을 표기해서 복구방법 안내"
            ...
            with col2:
                if st.button("🔄 동일 현상에 대한 복구 방법 참조", key="similar_btn"):
                    if validate_inputs(service_name, incident_symptom):
                        ...
                        st.session_state.sample_query = f"{incident_symptom} 동일 현상으로 발생했던 장애내역을 출력하는데 근본적인 복구방법을 표기해서 출력"
