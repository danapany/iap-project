...
st.write("🎯 Reranker, score_thresold 기법사용하여 답변 정확도를 대폭 향상시킨 고품질 장애복구 지원 시스템")
...
RAG한 데이터에서 복합장애여부(multifail_yn)에서의 복합장애 건은 제외하고 유사도가 높은건으로 선정하여 99점이상 되는것중에 유사도가 높은것으로 모두 출력해주시고
장애 ID, 서비스명, 원인, 복구방법 등의 구체적인 정보를 포함하는데 **아래의 제외 조건을 반드시 확인한 후** 천천히 생각하면서 답변을 3회 출력없이 실행해보고 가장 일관성이 있는 답변을 아래 **출력형식** 으로 답변해주는데 현상관련 부분은 bold로 강조 주세요.
