...
### 1단계: 장애내역 (Incident Records) 먼저 출력

**📋 장애내역 상세**

| 장애 ID | 서비스명 | 장애등급 | 발생일자 | 시간대 | 장애시간 | 장애현상 | 복구방법 | 담당부서 |
|---------|----------|----------|----------|--------|----------|----------|----------|----------|
| INM24... | xxx | 3등급 | 2024-04-01 | 주간 | 15분 | ... | ... | ... |
| INM24... | yyy | 2등급 | 2024-04-02 | 야간 | 30분 | ... | ... | ... |

### 2단계: 이상징후내역 (Anomaly Records) 이후 출력

**📋 이상징후내역 상세**

| 장애 ID | 서비스명 | 장애등급 | 발생일자 | 시간대 | 장애시간 | 장애현상 | 복구방법 | 담당부서 |
|---------|----------|----------|----------|--------|----------|----------|----------|----------|
| INM25... | zzz | 3등급 | 2025-01-10 | 주간 | 94분 | ... | ... | ... |
| INM25... | www | 2등급 | 2025-01-11 | 야간 | 45분 | ... | ... | ... |

**절대 준수사항:**
- 반드시 장애내역을 먼저 출력하고, 그 다음 이상징후내역을 출력
- 각 표의 제목에 출처를 명시 (🔴 장애내역, 🟡 이상징후내역)
- 순서를 바꾸거나 섞어서 출력하지 말 것
...
**최종 확인사항:**
1. ✅ 복구방법 박스에서 장애내역과 이상징후내역을 명확히 구분했는가?
2. ✅ 세부내역에서 장애내역을 먼저 출력하고 이상징후내역을 나중에 출력했는가?
3. ✅ 각 섹션의 제목에 출처를 명시했는가?
4. ✅ incident_repair 필드만 복구방법으로 사용했는가?
5. ✅ 모든 원본 데이터를 변경하지 않고 그대로 유지했는가?
"""
...
**장애내역과 이상징후내역이 모두 있는 경우 반드시 구분하여 표시:**
...
| 장애ID | 서비스명 | 장애등급 | 발생일자 | 시간대 | 요일 | 장애시간(분) | 장애현상 | 장애원인 | 담당부서 |
|--------|----------|----------|----------|--------|------|-------------|----------|----------|----------|
| [모든 내역을 한 줄씩 추가 - 각 필드는 원본 데이터 그대로] |
...
| 장애ID | 서비스명 | 장애등급 | 발생일자 | 시간대 | 요일 | 장애시간(분) | 장애현상 | 장애원인 | 담당부서 |
|--------|----------|----------|----------|--------|------|-------------|----------|----------|----------|
| INM25010112345 | ERP | 2등급 | 2025-01-15 | 주간 | 월요일 | 45 | 이지 접속 불가 | WAS의 Heap 메모리 설정값 초과로 인한 OOM 오류 | 재무DX개발팀 |
| INM25010212346 | ERP | 3등급 | 2025-01-16 | 야간 | 화요일 | 30 | 데이터베이스 연결 실패 | 커넥션 풀 고갈 | 시스템운영팀 |
