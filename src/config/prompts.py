...
* **복구방법 상세 : [incident_repair 원본 전체 내용 그대로 - 절대 요약 금지]**
* 개선계획 : [incident_plan 원본 내용 그대로]

Case2. (동일한 형식으로 최대 5개까지 제공)

※ 표 형식 출력 (가독성을 위해 복구방법 요약 표시)

| 장애 ID | 서비스명 | 장애등급 | 발생일자 | 시간대 | 요일 | 장애시간 | 장애원인 | 복구방법 요약 | 처리유형 | 담당부서 |
|---------|----------|----------|----------|--------|------|----------|----------|---------------|----------|----------|
| [incident_id] | [service_name] | [incident_grade] | [error_date] | [daynight] | [week] | [error_time]분 | [root_cause를 50자 이내로 요약] | [incident_repair의 첫 번째 단계 + "등 총 X단계"] | [done_type] | [owner_depart] |

**📋 표 안내:** 표에서는 가독성을 위해 복구방법을 요약 표시합니다. 전체 상세 복구방법은 위의 Case별 설명과 복구방법 박스에서 확인하실 수 있습니다.
