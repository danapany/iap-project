        ...
        with col2:
            ...
            with toggle_col2:
                internet_search_enabled = st.toggle(
                    ...
                    help="활성화하면 내부 학습데이터에서 답을 찾지 못할 때 인터넷에서 추가 정보를 검색합니다."
                )
            ...
            if internet_search_enabled:
                st.success("🌐 인터넷 검색이 **활성화**되었습니다. 내부 학습데이터 및 인터넷 검색을 수행하여 답변합니다.")
            else:
                st.info("📋 인터넷 검색이 **비활성화**되었습니다. 내부 학습데이터 만을 사용하여 답변합니다.")
            
            # SerpApi 설정 상태 확인
            from config.settings import AppConfig
            config = AppConfig()
            if not config.has_serpapi_config():
                st.warning("⚠️ SerpApi가 설정되지 않아 인터넷 검색 기능을 사용할 수 없습니다. 토글을 활성화해도 내부 학습데이터만 사용됩니다.")
    ...
    def show_config_error(self, env_status):
        ...
        **SerpApi 설정 방법:**
        ...
        if "❌" in env_status.get("SERPAPI_API_KEY", ""):
            st.warning("⚠️ SerpApi가 설정되지 않으면 내부 학습데이터에서 정보 부족 시 인터넷 검색을 사용할 수 없습니다.")
